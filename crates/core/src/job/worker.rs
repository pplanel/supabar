use crate::runtime::{ClientQuery, Context, Event, InternalEvent};

use super::jobs::{Job, JobReport, JobReportUpdate, JobStatus};
...
impl Worker {
    ...
    pub async fn spawn(worker: Arc<Mutex<Self>>, ctx: &Context) {
        ...
        while let Some(command) = channel.recv().await {
            ...
            match command {
                WorkerEvent::Progressed(changes) => {
                    ...
                    ctx.emit(Event::InvalidateQueryDebounced(ClientQuery::JobGetRunning))
                        .await;
                }
                WorkerEvent::Completed => {
                    ...
                    ctx.emit(Event::InvalidateQuery(ClientQuery::JobGetRunning))
                        .await;
                    ctx.emit(Event::InvalidateQuery(ClientQuery::JobGetHistory))
                        .await;
                    break;
                }
                WorkerEvent::Failed => {
